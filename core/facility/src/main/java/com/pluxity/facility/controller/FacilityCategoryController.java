...
import com.pluxity.facility.service.FacilityCategoryService;
...
public class FacilityCategoryController {
    ...
    @Operation(
            summary = "시설 카테고리 생성",
            description = "새로운 시설 카테고리를 생성합니다."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "시설 카테고리 생성 성공",
    ...
    @PostMapping
    @ResponseCreated
    public ResponseEntity<CreatedResponseBody<Long>> createFacilityCategory(
            @Parameter(description = "시설 카테고리 생성 정보", required = true)
            @Valid @RequestBody FacilityCategoryCreateRequest req) {
        FacilityCategoryResponse response = service.create(req);
        return ResponseEntity.ok(CreatedResponseBody.of(response.id()));
    }

    @Operation(
            summary = "시설 카테고리 목록 조회",
            description = "모든 시설 카테고리 목록을 조회합니다."
    )
    @ApiResponses(value = {
    ...
    @Operation(
            summary = "시설 카테고리 상세 조회",
            description = "ID를 기반으로 시설 카테고리의 상세 정보를 조회합니다."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<DataResponseBody<FacilityCategoryResponse>> getFacilityCategory(
            @Parameter(description = "시설 카테고리 ID", required = true)
    ...
    @Operation(
            summary = "시설 카테고리 수정",
            description = "ID를 기반으로 시설 카테고리를 수정합니다."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<Void> patchFacilityCategory(
            @Parameter(description = "시설 카테고리 ID", required = true)
            @PathVariable Long id,
            @Parameter(description = "시설 카테고리 수정 정보", required = true)
            @Valid @RequestBody FacilityCategoryUpdateRequest request) {
    ...
    @Operation(
            summary = "시설 카테고리 삭제",
            description = "ID를 기반으로 시설 카테고리를 삭제합니다."
    )
    @ApiResponses(value = {
