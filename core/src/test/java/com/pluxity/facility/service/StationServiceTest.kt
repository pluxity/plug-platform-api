...
internal class StationServiceTest {
    ...
    @Autowired
    lateinit var floorRepository: FloorRepository

    @Autowired
    lateinit var lineRepository: LineRepository

    @Autowired
    lateinit var stationLineRepository: StationLineRepository

    @Autowired
    lateinit var stationCodeRepository: StationCodeRepository
    ...
    fun save_WithValidRequest_SavesStationAndAllRelations() {
        ...
        val line2Id = createAndSaveLine("2호선", "LINE_2").id!!
        val lineSinbundangId = createAndSaveLine("신분당선", "LINE_SIN").id!!
        ...
        val stationCodes =
            stationCodeRepository.findAll()
                .filter { it.station.id == savedStation.id }

        Assertions.assertThat(stationCodes)
            .hasSize(2)
    ...
    fun save_WithDuplicateCode_ThrowsCustomException() {
        ...
        assertThatThrownBy {
            stationService.save(duplicateRequest)
    ...
    fun findAll_ReturnsListOfDetailedResponses() {
        ...
        val line1Id = createAndSaveLine("1호선", "L1").id!!
        stationService.save(
            StationCreateRequest(
    ...
    fun putUpdate_FullReplace_ReplacesAllFields() {
        // GIVEN: 원본 데이터 생성
        val lineAId = createAndSaveLine("A노선", "LA").id!!
        val lineBId = createAndSaveLine("B노선", "LB").id!!
        val stationId =
            stationService.save(
                StationCreateRequest(
        ...
        val lineCId = createAndSaveLine("C노선", "LC").id!!
        ...
        Assertions.assertThat(stationLines)
            .hasSize(2)
    ...
    fun delete_RemovesStationAndAllAssociatedRelations() {
        ...
        val lineId = createAndSaveLine("삭제될노선", "DEL_L").id!!
        val stationId =
            stationService.save(
                StationCreateRequest(
    ...
    fun addLineToStation_AddsRelationAndIgnoresDuplicate() {
        ...
        val lineId = createAndSaveLine("테스트노선", "T_L").id!!
    ...
    fun removeLineFromStation_RemovesRelation() {
        // GIVEN: 노선이 연결된 역 생성
        val lineId = createAndSaveLine("제거될노선", "REM_L").id!!
        val stationId =
            stationService.save(
                StationCreateRequest(
    ...
    private fun createAndSaveLine(
        ...
        return lineRepository.save(Line(name = name, color = color))
    }
