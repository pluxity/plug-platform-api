...
class RoleServiceTest {
    ...
    void save_withPermissionGroups_andVerifyWithService() {
        ...
        RoleCreateRequest createRequest =
                new RoleCreateRequest("Test Role", "A role for testing", initialGroupIds);
        ...
        List<Long> responseGroupIds =
                roleService.findRoleById(roleId).getRolePermissions().stream()
    ...
    void findById_returnsRoleWithAllPermissionsInGroups() {
        ...
        List<Long> initialGroupIds = List.of(permissionGroupIds.get(0), permissionGroupIds.get(1));
        RoleCreateRequest createRequest =
                new RoleCreateRequest("Test Role", "For findById test", initialGroupIds);
        Long roleId = roleService.save(createRequest);
        em.flush();
        em.clear();
        ...
        List<String> responseResourceIds =
        ...
        assertThat(responseResourceIds)
                .containsExactlyInAnyOrder(
                        String.valueOf(buildings.get(0).getId()), String.valueOf(buildings.get(1).getId()));
    }
    ...
    void update_andVerifyWithService() {
        ...
        Long roleId =
                roleService.save(
                        new RoleCreateRequest(
                                "Initial Role",
                                "Desc",
                                List.of(permissionGroupIds.get(0), permissionGroupIds.get(1))));
        em.flush();
        em.clear();
        ...
        RoleUpdateRequest updateRequest =
                new RoleUpdateRequest("Updated Role", "Updated Description", updatedGroupIdList);
        ...
        assertThat(finalResourceIds)
                .containsExactlyInAnyOrder(
                        String.valueOf(buildings.get(1).getId()), String.valueOf(buildings.get(2).getId()));
    }
    ...
    void delete_andVerifyDeletionWithService() {
        // GIVEN
        Role role =
                roleRepository.save(Role.builder().name("Deletable Role").description("Desc").build());
        em.flush();
        em.clear();
        PermissionGroup permissionGroup =
                permissionGroupService.findPermissionGroupById(permissionGroupIds.getFirst());
        List<RolePermission> newRolePermissions =
                List.of(RolePermission.builder().permissionGroup(permissionGroup).role(role).build());
