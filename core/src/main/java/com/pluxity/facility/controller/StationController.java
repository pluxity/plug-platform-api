...
import com.pluxity.facility.service.StationService;
...
public class StationController {
    ...
    public ResponseEntity<Long> create(
            @Parameter(description = "스테이션 생성 정보", required = true)
    ...
    public ResponseEntity<DataResponseBody<StationResponse>> get(
            @Parameter(description = "스테이션 ID", required = true)
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "이력 조회 성공",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = DataResponseBody.class))),
            @ApiResponse(responseCode = "404", description = "해당 ID의 스테이션을 찾을 수 없음",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class))),
            @ApiResponse(responseCode = "500", description = "서버 오류",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class)))
    })
    @GetMapping("/{id}/history")
    public ResponseEntity<DataResponseBody<List<FacilityHistoryResponse>>> getStationHistoryById(@PathVariable Long id) {
    ...
    public ResponseEntity<Void> patch(
            @Parameter(description = "스테이션 ID", required = true)
            @PathVariable Long id, 
            @Parameter(description = "스테이션 수정 정보", required = true)
