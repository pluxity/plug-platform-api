...
class StationLineService(
    ...
    fun findLinesByStation(station: Station): List<Long> {
        return stationLineRepository.findByStationOrderByCreatedAtDesc(station).map { it.line.id!! }
    }
    ...
    fun findLineMapByStationIds(stations: List<Station>): Map<Station, List<Long>> {
        ...
        return stationLineRepository.findByStationInOrderByCreatedAtDesc(stations)
            .groupBy { it.station }
