...
public class RoleController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "인증되지 않은 요청",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class))),
            @ApiResponse(responseCode = "409", description = "이미 존재하는 역할명",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class))),
            @ApiResponse(responseCode = "500", description = "서버 오류",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class)))
    })
    ...
    public ResponseEntity<Long> createRole(
            @Parameter(description = "역할 생성 정보", required = true)
    ...
    public ResponseEntity<Void> updateRole(
            @Parameter(description = "역할 ID", required = true)
            @PathVariable(name = "id") Long id, 
            @Parameter(description = "역할 수정 정보", required = true)
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "역할을 찾을 수 없음",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class))),
            @ApiResponse(responseCode = "409", description = "사용 중인 역할 삭제 시도",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class))),
            @ApiResponse(responseCode = "500", description = "서버 오류",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponseBody.class)))
    })
