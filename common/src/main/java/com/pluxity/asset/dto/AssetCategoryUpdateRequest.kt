...
data class AssetCategoryUpdateRequest(
    @field:Schema(description = "카테고리 이름", example = "그래픽 에셋") @param:Schema(
        description = "카테고리 이름",
        example = "그래픽 에셋"
    ) val name: @NotBlank(message = "카테고리 이름은 필수입니다") @Size(max = 50, message = "카테고리 이름은 50자를 초과할 수 없습니다") String?,
    @field:Schema(description = "카테고리 코드", example = "GRAPHIC_ASSET") @param:Schema(
        description = "카테고리 코드",
        example = "GRAPHIC_ASSET"
    ) val code: @NotBlank(message = "카테고리 코드는 필수입니다") @Size(max = 50, message = "카테고리 코드는 50글자를 초과할 수 없습니다") String?,
    @field:Schema(description = "부모 카테고리 ID", example = "1") @param:Schema(
        description = "부모 카테고리 ID",
        example = "1"
    ) val parentId: Long?,
    @field:Schema(description = "아이콘 파일 ID", example = "10") @param:Schema(
        description = "아이콘 파일 ID",
        example = "10"
