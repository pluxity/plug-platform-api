...
class AssetCategoryServiceTest {
    ...
    void setUp() {
        ...
        void getAssetCategory_withExistingId_returnsCorrectResponse() {
            ...
            // WHEN
            AssetCategoryResponse response = assetCategoryService.getAssetCategory(rootId);

            // THEN
            assertThat(response).isNotNull();
            assertThat(response.id()).isEqualTo(rootId);
            assertThat(response.name()).isEqualTo("가전");
            assertThat(response.thumbnail()).isNotNull(); // FileService Mocking으로 썸네일 포함 확인
        }
