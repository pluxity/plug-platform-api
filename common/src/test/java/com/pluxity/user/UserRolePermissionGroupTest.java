...
class UserRolePermissionGroupTest {
    ...
    void setUp() {
        ...
        mainFacilityGroupId = permissionGroupService.create(new PermissionGroupCreateRequest(
                "주요 시설 관리 그룹", null, List.of(new PermissionRequest("FACILITY", List.of("1", "2")))));
        subFacilityGroupId = permissionGroupService.create(new PermissionGroupCreateRequest(
                "보조 시설 관리 그룹", null, List.of(new PermissionRequest("FACILITY", List.of("3")))));
        deviceCategoryGroupId = permissionGroupService.create(new PermissionGroupCreateRequest(
                "장비 분류 조회 그룹", null, List.of(new PermissionRequest("DEVICE_CATEGORY", List.of("1", "2")))));
        ...
        operatorRoleId = roleService.save(new RoleCreateRequest("OPERATOR", "운영자",
                List.of(mainFacilityGroupId))); // 운영자는 주요 시설(1, 2)만 관리
        viewerRoleId = roleService.save(new RoleCreateRequest("VIEWER", "조회자",
                List.of(deviceCategoryGroupId))); // 조회자는 장비 분류(1, 2)만 조회

        // 3. Users 생성 및 Roles 할당
        adminUserId = userService.save(new UserCreateRequest("admin", "pw", "관리자 유저", null, null, null, List.of(adminRoleId))).id();
        operatorUserId = userService.save(new UserCreateRequest("operator", "pw", "운영자 유저", null, null, null, List.of(operatorRoleId))).id();
    ...
    class FullLifecycleScenario {
        ...
        void fullLifecycle_shouldMaintainConsistency() {
            ...
            permissionGroupService.update(mainFacilityGroupId, new PermissionGroupUpdateRequest(
                    "주요 시설 관리 그룹 v2", null, List.of(new PermissionRequest("FACILITY", List.of("2", "3")))));
            em.flush();
            em.clear();
            ...
            roleService.update(operatorRoleId, new RoleUpdateRequest(
                    "운영자+", null, List.of(mainFacilityGroupId, deviceCategoryGroupId)));
            em.flush();
            em.clear();
            ...
            userService.update(operatorUserId, new UserUpdateRequest(
                    null, null, null, null, List.of(viewerRoleId)));
            em.flush();
            em.clear();
    ...
    class AccessControlScenario {
        ...
        void users_shouldOnlyAccessTheirPermittedResources() {
            ...
            User operator = userRepository.findById(operatorUserId).orElseThrow();
            Long viewerUserId = userService.save(new UserCreateRequest("viewer", "pw", "조회자 유저", null, null, null, List.of(viewerRoleId))).id();
            em.flush();
            em.clear();
    ...
    class DeletionScenario {
        ...
        void whenPermissionGroupIsDeleted_accessShouldBeRevoked() {
            ...
            long count = rolePermissionRepository.findAll().stream()
                    .filter(rp -> rp.getPermissionGroup() != null && rp.getPermissionGroup().getId().equals(mainFacilityGroupId))
                    .count();
            ...
            assertFalse(operatorAfterDelete.canAccess("FACILITY", "1"), "PermissionGroup 삭제 후, 시설 접근은 불가능해야 합니다.");
        ...
        void whenRoleIsDeleted_userLosesPermissions() {
            ...
            long count = userRepository.findById(operatorBeforeDelete.getId()).get().getUserRoles().size();
        ...
        void whenUserIsDeleted_onlyUserRelatedDataShouldBeRemoved() {
            ...
            assertEquals(initialUserRoleCount - 1, userRoleRepository.count(), "UserRole 레코드가 1개 줄어야 합니다.");

            // 3. Role과 PermissionGroup은 영향을 받지 않았는지 확인
            assertTrue(roleRepository.existsById(operatorRoleId), "Role은 삭제되지 않아야 합니다.");
            assertTrue(permissionGroupRepository.existsById(mainFacilityGroupId), "PermissionGroup은 삭제되지 않아야 합니다.");
        }
    }
    ...
    class EdgeCaseScenario {
        ...
        void whenUserCreatedWithoutRole_thenAssignRole_shouldGrantPermissions() {
            // WHEN: Role 없이 사용자 생성
            UserCreateRequest createRequest = new UserCreateRequest("newUser", "pw", "신규 유저", null, null, null, null);
            ...
            userService.update(newUserId, new UserUpdateRequest(null, null, null, null, List.of(viewerRoleId)));
            em.flush();
            em.clear();
    ...
    class MultipleRolesScenario {
        ...
        void whenUserHasMultipleRoles_shouldAggregateAllPermissions() {
            ...
            Long multiRoleUserId = userService.save(new UserCreateRequest(
                    "multiRoleUser", "pw", "다중 역할 유저", null, null, null, List.of(operatorRoleId, viewerRoleId)
            )).id();
            em.flush();
            em.clear();
        ...
        void whenRoleHasOverlappingPermissions_canAccessShouldWorkCorrectly() {
            ...
            PermissionRequest overlappingPermission = new PermissionRequest("FACILITY", List.of("2", "4"));
            Long overlappingGroupId = permissionGroupService.create(new PermissionGroupCreateRequest(
                    "중복 권한 그룹", null, List.of(overlappingPermission)));

            // OPERATOR 역할에 이 그룹을 추가 (기존 '주요 시설 관리 그룹'과 "FACILITY:2"가 겹침)
            roleService.update(operatorRoleId, new RoleUpdateRequest(
                    null, null, List.of(mainFacilityGroupId, overlappingGroupId)));
            em.flush();
            em.clear();
